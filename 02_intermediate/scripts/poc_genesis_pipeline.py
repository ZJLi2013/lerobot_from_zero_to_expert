...
用法：
  python poc_genesis_pipeline.py               # 默认 GPU，自动查找 SO-101 XML
  python poc_genesis_pipeline.py --xml PATH    # 指定 XML 路径
  python poc_genesis_pipeline.py --cpu         # CPU fallback（无 GPU 时）
...
try:
    ...
    if rgb_np.ndim == 4:
        rgb_np = rgb_np[0]
    ok(f"cam.render() → RGB", f"shape={rgb_np.shape}, dtype={rgb_np.dtype}, range=[{rgb_np.min()},{rgb_np.max()}]")
    results["3"] = True
except Exception as e:
...
try:
    ...
    # 探测末端执行器链接名
    EE_CANDIDATES = [
        "gripper_link", "Fixed_Jaw", "end_effector", "tool0",
        "hand", "panda_hand",  # franka fallback
    ]
    ee_link = None
...
try:
    ...
    if USE_SO101:
        # SO-101: 6 DOF
        kp = np.array([100., 100., 80., 80., 60., 40.])[:n_dofs]
        kv = np.array([10.,  10.,  8.,  8.,  6.,  4.])[:n_dofs]
        HOME_DEG = np.array([0., -30., 90., -60., 0., 0.])[:n_dofs]
    else:
        # Franka: 9 DOF (7 arm + 2 finger)
        kp = np.array([4500, 4500, 3500, 3500, 2000, 2000, 2000, 100, 100], dtype=float)[:n_dofs]
...
if ee_link is None:
    ...
    try:
        ...
        qpos_np = qpos_target.cpu().numpy() if hasattr(qpos_target, 'cpu') else np.array(qpos_target)
        if qpos_np.ndim > 1:
            qpos_np = qpos_np[0]

        ok(f"IK solved", f"{dt_ik:.1f} ms, qpos={np.round(np.rad2deg(qpos_np), 1).tolist()} deg")
        results["6"] = True
...
try:
    ...
    qpos_np     = results.get("qpos_target", results.get("HOME_RAD", np.zeros(robot.n_dofs)))
...
try:
    ...
    ok(f"states  saved", f"shape={states.shape},  range=[{states.min():.1f}, {states.max():.1f}] deg")
    ok(f"actions saved", f"shape={actions.shape}, range=[{actions.min():.1f}, {actions.max():.1f}] deg")
    ok(f"images  saved", f"shape={images.shape},  range=[{images.min()}, {images.max()}]")
