...
Genesis 是目前最适合 SO-101 合成数据生成的仿真引擎：安装最简单（pip install genesis-world）、与 lerobot 接口天然兼容、并行仿真速度 10–80x 于 MuJoCo/Isaac Sim。

详细的可行性评估、架构设计、完整 Sample Code（单环境验证 + 并行批量采集）及落地步骤，见 **[genesis_sdg.md](./genesis_sdg.md)**。
