...
### 9.4 已确认的结构性阻塞（E42–M1, 2026-03-06）

#### 9.4.1 `grasp_center` 位置定义偏差

`so101_new_calib.xml` 中 `gripperframe` / `grasp_center` 的 `pos="-0.0079 -0.000218 -0.0981"` 把 TCP 放到了 **gripper body 下方约 8cm**，远低于实际夹爪尖端。
...
| gripper | 0.104 | +0.089 |
| moving_jaw | 0.090 | +0.075 |

IK 精确地把 `grasp_center` 送到了 cube，但夹爪在 cube 上方约 8cm，永远碰不到目标物。

#### 9.4.2 工作空间不可达

`diag_gc_sweep.py` 对 `grasp_center` 目标 z 做全范围扫描（-0.08 至 +0.115m）：

- IK 精度 < 0.005m 的区间：`target_z >= 0.010`
- 在所有 GOOD_IK 区间内，夹爪中点最低 = **0.073m**
- cube top = 0.030m，gap >= 4.3cm

结论：**SO-101 在当前 MJCF 下，夹爪机构无法下探到 z < 0.07m。**

#### 9.4.3 尝试过但无效的修复

| 方案 | 实验 | 结果 |
|------|------|------|
| 移动 grasp_center 到夹爪中点 | diag_link_positions | IK 误差从 0.0002m 退化到 0.077m |
| 加 platform 抬高 cube | P1-P5 | 偶发 trial 正向 Δz，重复 10 次 → 0/10 可复现 |
| lateral approach 避免碰撞 | P3 | 避免了撞落 cube，但无接触 |
| `quat=[0,1,0,0]` 朝向约束 | O1-O2 | 5-DOF 手臂无法同时满足位置+朝向，误差 > 0.06m |
| 缩短 episode 匹配 trial | E43-E44 | trial 偶发正向 Δz，full-episode 均失败 |
...
> 核心阻塞是 MJCF `gripperframe` site 位置错误 + 手臂工作空间限制，非调参可解。

后续推荐方向（按优先级）：

1. **重新标定 `grasp_center` 位置**：用真实机器人或 CAD 数据，把 `grasp_center` body 放到夹爪 pinch center（约 `pos=(0.01, 0.01, -0.025)` 量级，具体需实测），同时确认 IK 在该位置仍然可达
2. **提高物体高度到工作空间内**：使用 platform 让 cube center 进入 jaw 可达区间（z > 0.10m）
3. **侧向抓取策略**：放弃 top-down pick，改为水平方向 push-and-pinch
4. **多 DOF 升级**：SO-101 只有 5+1 DOF，无法同时约束位置和朝向；如需朝向控制考虑 7-DOF 手臂
